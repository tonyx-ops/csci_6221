...
struct UserProfileView: View {
    ...
    private var isFollowing: Bool {
        guard let currentUserId = authViewModel.currentUser?.id,
              let profileUserId = user.id else { return false }
        return authViewModel.currentUser?.following.contains(profileUserId) ?? false
    }
    
    // MARK: - Methods
    
    private func toggleFollow() {
        guard let profileUserId = user.id else { return }
