...
struct UserProfileView: View {
    ...
    init(user: User? = nil, isOwnProfile: Bool) {
        if let user = user {
            self._user = State(initialValue: user)
        } else {
            self._user = State(initialValue: User(id: "", email: ""))
        }
    ...
    var body: some View {
        ...
        .onAppear {
            if isOwnProfile {
                if let current = authViewModel.currentUser {
                    self.user = current
                }
            }
        }
    ...
    // MARK: - Methods
        ...
        private func toggleFollow() {
            ...
            Task {
                ...
                if let updated = await authViewModel.getUser(by: profileUserId) {
                    self.user = updated
                }
            }
        }
